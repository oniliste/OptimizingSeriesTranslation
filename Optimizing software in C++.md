...
*Intel* 提供了各种支持多线程软件的开发工具。参见[**《Intel 技术期刊》**2007年第11卷第4期](www.intel.com/technology/itj/)。
...
2. 如果一个函数使用 *CPU 分派*，在多个版本中使用或者不使用  *AVX*  支持进行编译，那么在离开 *AVX*  部分之前调用 `_mm256_zeroupper()`。
...
这使得*CPU 分派* 更加复杂。你可以选择对特定任务有用的扩展，并为具有此扩展的处理器创建代码分支。
...
目前可以使用几种不同的预定义的向量类库，包括一个来自 *Intel*的，一个来自我编写的。我编写的向量类库（*VCL* ）有许多特性，请参见[www.agner.org/optimize/#vectorclass](www.agner.org/optimize/#vectorclass)。**Intel vector class library** 最近没有更新，我觉得可能有些过时。
...
下面是一些长向量数学库的列表：
1. Intel vector math library (VML, MKL)。支持所有*x86* 平台。这个库降低了*非英特尔CPU* 的性能，除非你重写了英特尔的 *CPU 分派程序*。见**13.7 Intel 编译器中的 CPU分派**。
